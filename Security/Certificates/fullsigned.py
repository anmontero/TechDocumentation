...
CN1 = input("Enter the CN value: ")
dns_entries = int(input("How many DNS entries do you need? "))
dns_list = []

for i in range(dns_entries):
    dns = input(f"Enter DNS entry {i+1}: ")
    dns_list.append(dns)

with open(f'out/req.cnf', 'w') as f:
    f.write("""
[req]
distinguished_name = req_distinguished_name
...
""".format(CN1=CN1))
    for i, dns in enumerate(dns_list):
        f.write(f"DNS.{i+1} = {dns}\n")

# generate the leaf keyfile
subprocess.run(['openssl', 'genrsa', '-out', 'out/leaf.key', '2048'])
subprocess.run(['openssl', 'req', '-new', '-key', 'out/leaf.key', '-out', 'out/leaf.csr', '-config', 'out/req.cnf'])
subprocess.run(['openssl', 'x509', '-req', '-days', '365', '-in', 'out/leaf.csr', '-CA', 'intermediate/certs/intermediate.crt', '-CAkey', 'intermediate/private/intermediate.key', '-CAcreateserial', '-out', 'out/leaf.crt', '-extensions', 'req_ext', '-extfile', 'out/req.cnf'])